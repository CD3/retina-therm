...
def test_large_beam_call_function():
    ...
    assert G(Q_(0, "cm"), Q_(0, "s")).to("K/s") == Q_(1 / 2, "K/s")
    assert G(Q_(1, "cm"), Q_(0, "s")).to("K/s").magnitude == pytest.approx(
        Q_(1 / 2, "K/s") * math.exp(-1)
    )
    assert G(Q_(1, "cm"), Q_(1, "s")).to("K/s").magnitude == pytest.approx(
...
def test_axial_part_retina_long_time_approx():
    ...
    assert G_exact(0, 0.0) == pytest.approx(G_approx(0, 0.0))
    assert G_exact(0, 0.0) == pytest.approx(
        G_approx_with_units(Q_(0, "cm"), Q_(0.0, "s")).magnitude
    )
    assert type(G_exact(0, 0.0)) == mp.mpf
    assert type(G_approx(0, 0.0)) == numpy.float64

    assert G_exact(0, 0.01) == pytest.approx(G_approx(0, 0.01), rel=0.01)
    assert G_exact(0, 0.01) == pytest.approx(
        G_approx_with_units(Q_(0, "cm"), Q_(0.01, "s")).magnitude, rel=0.01
    )
    assert type(G_exact(0, 0.01)) == mp.mpf
    assert type(G_approx(0, 0.01)) == numpy.float64

    with mp.workdps(200):
        assert G_exact(0, 0.2) == pytest.approx(G_approx(0, 0.2), rel=0.01)
        assert G_exact(0, 0.2) == pytest.approx(
            G_approx_with_units(Q_(0, "cm"), Q_(0.2, "s")).magnitude, rel=0.01
        )
        assert type(G_exact(0, 0.2)) == mp.mpf
        assert type(G_approx(0, 0.2)) == numpy.float64

    with mp.workdps(2000):
        assert G_exact(0, 2) == pytest.approx(G_approx(0, 2), rel=0.01)
        assert G_exact(0, 2) == pytest.approx(
            G_approx_with_units(Q_(0, "cm"), Q_(2, "s")).magnitude, rel=0.01
        )
        assert type(G_exact(0, 2)) == mp.mpf
        assert type(G_approx(0, 2)) == numpy.float64

    with mp.workdps(200):
        assert G_exact(-0.001, 0.2) == pytest.approx(G_approx(-0.001, 0.2), rel=0.01)
        assert G_exact(-0.001, 0.2) == pytest.approx(
            G_approx_with_units(Q_(-10, "um"), Q_(0.2, "s")).magnitude, rel=0.01
        )
...
def test_multi_layer_greens_function_calcs():
    ...
    assert G1(1, 1) == pytest.approx(G2(1, 1))
